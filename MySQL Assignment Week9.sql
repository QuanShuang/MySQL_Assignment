...
-- 6b. join to display total amount rung up by each staff in aug 2005, staff&payment
select s.username, sum(p.amount) as "Total amount in Aug-2005"
from payment as p
...
-- actually I found that all these films have language_id = 1 which is English.
select film.title from film
where language_id in (select language_id from language where name = "English")
and title like "K%"
or title like "Q%"; 

-- 7b. display all actors in film Alone Trip
select first_name, last_name from actor
where actor_id in 
...
-- 7c. Email marketing campaign. Need names and email add of all Ca customers. Use join
